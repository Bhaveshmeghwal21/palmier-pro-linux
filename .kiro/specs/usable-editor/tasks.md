...
- [ ] 1. Wire timeline selection to the Inspector (Requirement 1) — **XS**
  - [ ] 1.1 Connect `TimelinePanel`'s `QTreeView` selection to `InspectorViewModel::selectClip()`,
  ...
  - [ ] 1.2 Clear the Selection when the selected clip disappears from the project by any surface, so
        the Inspector never points at an absent clip.
  - [ ] 1.3 Replace the after-the-fact "No clip is selected" status message on `Edit > Delete Clip`
        and `Edit > Split at Playhead` with proper enable/disable state driven by the Selection.
  - [ ] 1.4 Tests: selecting a clip row populates the Inspector; selecting a track row clears it;
        removing the selected clip clears it; both Edit actions are disabled with no Selection and
        enabled with one.

- [ ] 2. Add track creation to the GUI (Requirement 2) — **XS**
  - [ ] 2.1 Add an `addTrack` gesture to `TimelineViewModel` routed through `GuiToolGateway::addTrack`
        (the gateway method already exists; the view model has only `canAddTrack()`).
  - [ ] 2.2 Add "Add Video Track" and "Add Audio Track" actions to the menu bar, disabled when
        `canAddTrack()` is false.
  - [ ] 2.3 Tests: each action appends after the last track of its kind; both are disabled at the
        track ceiling; each is undoable in exactly one Undo.

- [ ] 3. Add clip placement to the GUI (Requirement 3) — **S**
  - [ ] 3.1 Add a "Place at Playhead" gesture that takes the Media_Browser's selected asset and the
        Timeline_Panel's selected track and calls the existing `GuiToolGateway::addClip`.
  - [ ] 3.2 Disable it when no asset is selected or no track exists; surface an overlap refusal as a
  ...
  - [ ] 3.3 Implement drag-and-drop from the Media_Browser list onto a Timeline_Panel track row as an
        equivalent route (the UI layer currently has no drag-and-drop code of any kind).
  - [ ] 3.4 Tests: placement produces project state equal to invoking `timeline.add_clip` through the
        MCP endpoint with the same arguments; overlap is refused with no change; drag-and-drop and the
        menu route produce identical state; each placement is one Undo.

- [ ] 4. Add playhead positioning (Requirement 4) — **S**
  - [ ] 4.1 Add a scrubbable playhead control and an editable timecode field to the Timeline_Panel,
        both driving the existing `PreviewView::seekSeconds`.
  - [ ] 4.2 Add frame-step-back and frame-step-forward actions using the project frame rate's frame
        interval.
  - [ ] 4.3 Snap every requested position to the nearest frame boundary; clamp out-of-range requests to
        the nearer bound.
  - [ ] 4.4 Tests: the displayed timecode agrees with the playhead to the frame; stepping moves exactly
        one frame interval; negative and beyond-duration requests clamp rather than error.

- [ ] 5. **Phase 1 checkpoint** — the usability threshold's first half
  - [ ] 5.1 An end-to-end GUI test that, driving only widgets: creates a track, imports a fixture
        ...
        This is the test that proves a GUI user can author from nothing, which is impossible today.
  - [ ] 5.2 Extend the `ci.yml` launch smoke test to drive this sequence with `xdotool` and capture a
        screenshot of the populated timeline, so the artifact shows a real edit rather than an empty
        project.
  - [ ] 5.3 Update `docs/UPSTREAM_PARITY.md`'s `timeline editing` row, which is the report's only
        `must`-priority gap.
...
Nothing in this plan has been started. The audit in `requirements.md` was verified against commit
`1bc20d1` on `main`, at which point CI is fully green (run `32362473786`) with 1239 of 1239 tests
passing and the launch smoke test capturing a rendered — but empty and uneditable — editor.

**The single most valuable finding, restated because it determines the whole plan:** GUI editing is
blocked by four unwired connections, not by missing features. `InspectorViewModel::selectClip()`,
`GuiToolGateway::addTrack()` and `GuiToolGateway::addClip()` are all implemented and tested but have
no callers, and no drag-and-drop or context-menu code exists anywhere in `src/ui/`. Task 1.1 alone — a
single signal connection — unblocks clip deletion, splitting, trimming, opacity, gain and effects, all
of which are already built. That is why Phase 1 is labelled XS/S throughout while delivering the
largest usability change in the document.
