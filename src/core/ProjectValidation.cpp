...
Result<void> validateClip(const Clip& clip) {
    ...
    // Requirement 10: a caption cue's text must not be empty — an empty cue
    // has nothing to burn in or write to a sidecar file, and is refused rather
    // than silently authored.
    if (clip.captionText.has_value() && clip.captionText->empty()) {
        return outOfRange("Clip " + clipLabel(clip) + ": captionText must not be empty");
    }
...
// Requirement 9/10: a clip's textStyle/captionText presence must agree with
// its track's kind — a TEXT track's clips must all carry a TextStyle and no
// other kind's clips may; a CAPTION track's clips must all carry captionText
// and no other kind's clips may. Shared by validateTrack() and
// validateProject()'s own inlined per-clip loop (see that function's own
// comment for why the check is duplicated there rather than delegated).
Result<void> checkClipKindAgreesWithTrackKind(const Track& track, const Clip& clip) {
    ...
    if (track.kind == TrackKind::Caption && !clip.isCaptionCue()) {
        return failedPrecondition("Clip " + clipLabel(clip) +
                                  ": on a caption track but carries no captionText");
    }
    if (track.kind != TrackKind::Caption && clip.isCaptionCue()) {
        return failedPrecondition("Clip " + clipLabel(clip) +
                                  ": carries captionText but its track is not a "
                                  "caption track");
    }
    return ok();
}
...
Result<void> validateTrack(const Track& track) {
    for (const Clip& clip : track.clips) {
        ...
        if (auto r = checkClipKindAgreesWithTrackKind(track, clip); !r) {
            return r;
        }
    }
    return ok();
...
Result<void> validateProject(const Project& project) {
    ...
    // non-caption clip. The track/clip-kind consistency check reuses the same
    // checkClipKindAgreesWithTrackKind() helper validateTrack() calls, rather
    // than a second, separate pass, because this loop already walks every
    // clip once for validateClip() and asset resolution; a second pass over
    // the same clips would just re-open the question of which error a caller
    // sees first for no benefit.
    for (const Track& track : project.tracks) {
        for (const Clip& clip : track.clips) {
            ...
            // Requirement 9/10: neither a text clip nor a caption cue has any
            // source media, so both are exempt from the
            // assetRef-resolves-in-Project.assets rule entirely.
            if (clip.isTextClip() || clip.isCaptionCue()) {
                continue;
            }
